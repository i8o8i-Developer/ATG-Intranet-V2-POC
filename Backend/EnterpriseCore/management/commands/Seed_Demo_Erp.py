...
class Command(BaseCommand):
    ...
    def seed_projects(self, employees):
        ...
        for ta in TeamAssignment.objects.filter(tenant=self.tenant):
            self.upsert(TeamAssignmentHistory, {"tenant": self.tenant, "team_assignment": ta, "action": "added"}, {"comment": "Initial Team Assignment", "changed_by": employees.get("EMP001"), "created_at": self.now - timezone.timedelta(days=30)})
