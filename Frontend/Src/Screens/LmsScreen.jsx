  ...
  return (
    <section className="lms-screen lms-command-center">
      <section className="lms-command-bar">
        ...
        <div className="lms-command-actions"><div className="lms-search"><Search size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search Lead, Owner, Source" /></div><button className="outline-button" onClick={reload}><RefreshCw size={16} /> Refresh</button></div>
      </section>
      ...
      <section className="lms-workspace-grid">
        ...
        <main className="lms-lead-console">
          <section className="lms-queue-head"><div><h2>Lead Queue</h2><span>{selected.size} Selected</span></div><div><button className="icon-button" title="Assign Selected"><Users size={16} /></button><button className="icon-button" title="More Actions"><MoreHorizontal size={16} /></button></div></section>
