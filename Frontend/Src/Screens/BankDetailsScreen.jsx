...
export function BankDetailsScreen({ data, selectedEmployeeId, reload }) {
  ...
  return <section className="Form-Page"><h1>Bank Details</h1><label>Employee<input value={employee.display_name || ""} readOnly /></label><label>Account IFSC<input value={form.ifsc} onChange={(event) => setForm({ ...form, ifsc: event.target.value })} /></label><label>Account Number<input type="password" value={form.account} onChange={(event) => setForm({ ...form, account: event.target.value })} /></label><label>Confirm Account Number<input value={form.confirm} onChange={(event) => setForm({ ...form, confirm: event.target.value })} placeholder="ConfirmBankAccountNumber" /></label><label>UPI<input value={form.upi} onChange={(event) => setForm({ ...form, upi: event.target.value })} /></label><button className="Primary-Button" onClick={save} disabled={!selectedEmployeeId || (form.confirm && form.confirm !== form.account)}>Update</button><Panel title="CurrentBankDetails"><SimpleTable columns={["AccountNumber", "IFSCCode", "UPIAddress", "Status"]} rows={(data.bankAccounts || []).filter((item) => !selectedEmployeeId || String(item.employee) === String(selectedEmployeeId)).map((item) => [item.masked_account_number, item.ifsc_code, item.upi_id, item.verification_status])} /></Panel></section>;
}