...
export function WorkflowIntelligenceScreen({ data, reload }) {
  ...
  const [form, setForm] = useState({ title: "WorkflowIntelligenceReport", report_type: "Manual", start_date: "", end_date: "" });
  ...
  return (
    <section className="Workflow-ScreenIntelligence-Screen">
      ...
      <section className="Intelligence-Kpi-Grid">
        ...
        <section><span>Top Workflow</span><strong>{topRows[0]?.hit_count || 0}</strong><small>{topRows[0]?.workflow_name || "NoTrafficYet"}</small></section>
      </section>
      ...
      <section className="Intelligence-Grid">
        ...
        <section className="Workflow-Radar">
          ...
          {!topRows.length && <EmptyState label="NoWorkflowUsageRowsReturnedYet." />}
        </section>
      </section>
      ...
      <section className="Workflow-Map-Grid">
        ...
        {!workflowMaps.length && <EmptyState label="NoWorkflowMapsReturned." />}
      </section>

      <section className="Intelligence-Bottom-Grid">
        <section className="Report-Feed"><header><h2>Reports</h2><span>{reports.length} Records</span></header>{reports.slice(0, 6).map((item) => <article key={item.id}><div><strong>{item.title}</strong><span>{item.report_type} / {item.status}</span></div><time>{formatDate(item.generated_for || item.created_at)}</time></article>)}{!reports.length && <EmptyState label="NoReportsGeneratedYet." />}</section>
        <section className="Route-Usage-Feed"><header><h2>Route Usage</h2><span>{summaryRows.length} Rows</span></header>{summaryRows.map((item) => <article key={`${item.workflow_name}-${item.route_pattern}-${item.username}`}><div><strong>{item.workflow_name}</strong><code>{item.route_pattern}</code></div><span>{item.username || "AllUsers"}</span><b>{item.hit_count}</b></article>)}</section>
      </section>
    </section>
  );
}