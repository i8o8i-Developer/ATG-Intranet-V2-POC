    ...
    <section className="Workflow-Screen Intelligence-Screen">
      <section className="Intelligence-Command">
        ...
        <div className="Intelligence-Actions">
          <button className="Outline-Button" onClick={reload}><RefreshCw size={16} /> Refresh</button>
          <button className="Primary-Button" onClick={generateReport}><BarChart3 size={16} /> Generate Report</button>
        </div>
      </section>
      ...
      <section className="Intelligence-Grid">
        <section className="Report-Builder">
          <header><FileText size={18} /><div><h2>Report Builder</h2><span>Persisted Workflow Report Output</span></div></header>
          <div className="Report-Form">
            <label>Title<input value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} /></label>
            <label>Type<select value={form.report_type} onChange={(event) => setForm({ ...form, report_type: event.target.value })}><option>Manual</option><option>Weekly</option><option>Audit</option><option>MCP Readiness</option></select></label>
            <label>Start Date<input type="date" value={form.start_date} onChange={(event) => setForm({ ...form, start_date: event.target.value })} /></label>
            <label>End Date<input type="date" value={form.end_date} onChange={(event) => setForm({ ...form, end_date: event.target.value })} /></label>
          </div>
        ...
        <section className="Workflow-Radar">
          ...
          {topRows.map((row) => (
            <div className="Radar-Row" key={row.workflow_name}>
              <div><strong>{row.workflow_name || "Unnamed"}</strong><span>{row.hit_count || 0} Route Hits</span></div>
              <Progress value={totalHits ? (Number(row.hit_count || 0) / totalHits) * 100 : 0} />
            </div>
          ))}
      ...
      <section className="Workflow-Map-Grid">
        {workflowMaps.map((item) => (
          <article className="Workflow-Map-Tile" key={item.id || `${item.owning_module}-${item.workflow_name}`}>
            <header><span>{item.owning_module}</span><b>{Array.isArray(item.route_patterns) ? item.route_patterns.length : 0} Routes</b></header>
            <h2>{item.workflow_name}</h2>
            <p>{item.description}</p>
            <div>{(item.route_patterns || []).slice(0, 3).map((route) => <code key={route}>{route}</code>)}</div>
          </article>
        ))}
      ...
      <section className="Intelligence-Bottom-Grid">
        <section className="Report-Feed">
          <header><h2>Reports</h2><span>{reports.length} Records</span></header>
          {reports.slice(0, 6).map((item) => (
            <article key={item.id}>
              <div><strong>{item.title}</strong><span>{item.report_type} / {item.status}</span></div>
              <time>{formatDate(item.generated_for || item.created_at)}</time>
            </article>
          ))}
          {!reports.length && <EmptyState label="No Reports Generated Yet." />}
        </section>
        <section className="Route-Usage-Feed">
          <header><h2>Route Usage</h2><span>{summaryRows.length} Rows</span></header>
          {summaryRows.map((item) => (
            <article key={`${item.workflow_name}-${item.route_pattern}-${item.username}`}>
              <div><strong>{item.workflow_name}</strong><code>{item.route_pattern}</code></div>
              <span>{item.username || "All Users"}</span>
              <b>{item.hit_count}</b>
            </article>
          ))}
        </section>
