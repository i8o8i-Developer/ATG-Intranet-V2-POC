...
export function McpScreen({ data, reload }) {
  ...
  return (
    <section className="Mcp-ScreenScreen-Stack">
      <section className="Screen-Header">
        ...
        <div className="Table-Actions">
          <button className="Primary-ButtonSmall" onClick={() => setCreateAgentOpen(true)}><Plus size={14} /> New Agent</button>
      ...
      <div className="Stat-GridFour">
        <StatCard label="AgentPrincipals" value={stats.agents} />
        <StatCard label="ToolDefinitions" value={stats.tools} />
        <StatCard label="Resources" value={stats.resources} />
        <StatCard label="AccessGrants" value={stats.grants} />
        <StatCard label="InvocationAudits" value={stats.audits} />
        <StatCard label="PendingDrafts" value={stats.drafts} />
      </div>
      ...
      <Tabs
        ...
        items={[
          ...
          ["resources", "Resources"],
          ["grants", "AccessGrants"],
          ["audits", "InvocationAudits"],
          ["drafts", "DraftActions"],
        ]}
      />

      {tab === "agents" && (
        <Panel title="AgentPrincipals">
              ...
              {agents.map((agent) => (
                <tr key={agent.id}>
                  ...
                  <td className="Table-Actions">
                    <button className="Soft-ButtonSmall" onClick={() => setInvokeAgent(agent)}>Invoke</button>
                    <button className="Soft-ButtonSmall" onClick={() => setDraftFor(agent)}>Draft Action</button>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
          {!agents.length && <EmptyState label="NoAgentPrincipalsYet." />}
        </Panel>
      )}

      {tab === "tools" && (
        <Panel title="ToolDefinitions">
          <SimpleTable
          ...
          />
          {!tools.length && <EmptyState label="NoToolsRegistered." />}
        </Panel>
      )}

      {tab === "resources" && (
        <Panel title="ResourceDefinitions">
          <SimpleTable
          ...
          />
          {!resources.length && <EmptyState label="NoResourcesRegistered." />}
        </Panel>
      )}

      {tab === "grants" && (
        <Panel title="AccessGrants" right={<button className="Primary-ButtonSmall" onClick={() => setGrantOpen(true)}><Plus size={13} /> New Grant</button>}>
          <SimpleTable
          ...
          />
          {!grants.length && <EmptyState label="NoGrantsYet." />}
        </Panel>
      )}

      {tab === "audits" && (
        <Panel title="RecentInvocations">
          <SimpleTable
          ...
          />
          {!audits.length && <EmptyState label="NoInvocationAuditsYet." />}
        </Panel>
      )}

      {tab === "drafts" && (
        <Panel title="DraftAgentActions">
          <table className="Erp-Table">
            ...
            <tbody>
              {drafts.map((draft) => (
                <tr key={draft.id}>
                  ...
                  <td className="Table-Actions">
                    {draft.status !== "Approved" && <button className="Soft-ButtonSmall" onClick={() => approveDraft(draft)}>Approve</button>}
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
          {!drafts.length && <EmptyState label="NoDraftActions." />}
        </Panel>
      )}
...
function CreateAgentModal({ data, onClose, reload }) {
  ...
  return (
    <Modal title="NewAgentPrincipal" onClose={onClose} wide>
...
function CreateGrantModal({ data, onClose, reload }) {
  ...
  return (
    <Modal title="NewAccessGrant" onClose={onClose} wide>
      <div className="Form-GridTwoModal-Form">
...
function InvokeAgentModal({ agent, data, onClose, reload }) {
  ...
  return (
    <Modal title={`Invoke As ${agent.display_name || agent.name}`} onClose={onClose} wide>
      ...
      <div className="Table-Actions">
        <button className="Soft-ButtonSmall" onClick={checkPermission} disabled={busy}><ShieldCheck size={13} /> Check Permission</button>
