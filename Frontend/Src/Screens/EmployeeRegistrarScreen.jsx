...
import { StatCard, StatusPill } from "./Shared/ScreenComponents.jsx";
    ...
    <section className="RG">
      ...
      <section className="RG-Layout">
        <div className="RG-Card">
          <div className="RG-CardHead"><h2>Onboarding Intake</h2><p>Create The Employee Record, Generate A One-Time Login, And Provision Credential Vault Entries.</p></div>
          <div className="RG-CardBody">
            <div className="RG-Form">
              ...
              <label>Password<div className="RG-CredInline"><input value={form.password} onChange={(event) => update("password", event.target.value)} /><button className="Outline-Button" type="button" onClick={() => update("password", generatePassword())}>Regenerate</button></div></label>
            ...
            <div className="RG-Pack">
              ...
              {credentialBlueprints.map((item) => (
                <label key={item.key} className="RG-CredRow">
                  <span className="RG-CredToggle"><input type="checkbox" checked={item.enabled} onChange={(event) => setCredentialOptions((current) => ({ ...current, [item.key]: event.target.checked }))} />{item.system_name}</span>
        ...
        <div className="RG-Card">
          <div className="RG-CardHead"><h2>Existing Access Pack</h2><p>Recent Credential Items Already Created In The Rebuilt Vault.</p></div>
          <div className="RG-CardBody">
            <table className="RG-Table">
              <thead><tr><th>System</th><th>Name</th><th>Owner</th><th>Status</th></tr></thead>
              <tbody>{recentCredentials.map((item) => <tr key={item.id}><td>{item.system_name}</td><td>{item.name}</td><td>{item.owner}</td><td>{item.status}</td></tr>)}</tbody>
            </table>
            {!recentCredentials.length && <div className="RG-Empty">No Credential Items Yet.</div>}
          </div>
        </div>
      </section>

      <section className="RG-Bottom">
        <div className="RG-Card">
          <div className="RG-CardHead"><h2>Recent Registered Employees</h2></div>
          <div className="RG-CardBody">
            <table className="RG-Table">
        ...
        {result && (
          <div className="RG-Result">
            <div className="RG-CardHead"><h2>Onboarding Result</h2><p>Mode: {result.mode}</p></div>
