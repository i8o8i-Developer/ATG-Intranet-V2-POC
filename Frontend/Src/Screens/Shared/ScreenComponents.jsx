...
export function Panel({ title, subtitle, right, children }) {
  return <section className="PanelGlassFade-In"><header><div><h2>{title}</h2>{subtitle && <p>{subtitle}</p>}</div>{right}</header><div>{children}</div></section>;
}

export function Disclosure({ title, subtitle, children, defaultOpen = true }) {
  const [open, setOpen] = useState(defaultOpen);
  return <section className="PanelDisclosureGlassFade-In"><header onClick={() => setOpen(!open)}><div><h2>{title}</h2>{subtitle && <p>{subtitle}</p>}</div>{open ? <ChevronDown /> : <ChevronRight />}</header>{open && <div>{children}</div>}</section>;
}
...
export function Modal({ title, children, onClose, wide }) {
  return <div className="Modal-Backdrop"><section className={wide ? "ModalWide" : "modal"}><header><h1>{title}</h1><button className="Icon-Button" onClick={onClose}><X /></button></header>{children}</section></div>;
}
